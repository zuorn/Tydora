...
> [!TIP]商店版与便携版的更新入口都在设置的「关于」标签；三个渠道共用同一份签名密钥与 latest.json。
...
> [!NOTE]桌面端在 2026-09 的重构中已从仓库根的 src-tauri/ 移到 app/tydora-desktop/，VERSION 与 tauri.conf.json 的同步由 npm run sync-version（scripts/sync-version.mjs）负责。
