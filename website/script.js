/* =================================================================
 * Tydora Official Website — Client Script (Terax.app-inspired)
...
(function () {
  ...
  const i18n = {
    cn: {
      ...
      'f1.desc': '基于 Vditor 3.x 引擎，支持三种编辑模式——所见即所得（WYSIWYG）、即时渲染（IR）和分屏预览（SV）。内置数学公式、代码高亮、打字机模式、实时大纲与字数统计。',
      ...
      'f2.desc': '强大的命令面板覆盖所有操作：编辑命令、格式命令、视图命令、文件命令，以及菜单命令、插件命令和快捷键速查。最近使用智能排序，所想即可达。',
      ...
      'f3.desc': '基于 Markmap 的思维导图视图，以可视化方式呈现文档结构。独立窗口自由缩放层级，让结构化思考一目了然。',
      ...
      'f4.desc': '7 款精心打磨的内置主题，同时兼容导入 Typora 主题 CSS 文件。编辑器主题可独立于应用外观设置。',
      'f4.defaultTag': '默认',
      ...
      'rm.r2Title': '图片增强',
      'rm.r2Desc': '粘贴拖拽上传、图床支持、多种存储模式、压缩裁剪与批量管理。',
      'rm.r3Title': '完整编辑器设置',
      ...
      'dl.title': '选一个版本开始用。',
      'dl.recommended': '推荐',
      'dl.tagIntel': 'Intel + Apple Silicon',
      'dl.tagDeb': '.deb · AppImage',
      'dl.winFormat': 'NSIS 安装包 · 64-bit',
      'dl.ctaWin': '下载 .exe',
      'dl.winReq': 'Windows 10 或更高版本（推荐 Windows 11）',
      'dl.macFormat': '.dmg 通用安装包',
      'dl.ctaMac': '下载 .dmg',
      'dl.macReq': 'macOS 11 Big Sur 或更高版本',
      'dl.linuxFormat': 'Debian 系发行版通用包',
      'dl.ctaLinux': '下载 .deb',
      'dl.linuxReq': 'Ubuntu 20.04 LTS 或更高版本',
    ...
    en: {
      ...
      'hero.ctaDownload': 'Download Tydora',
      ...
      'f1.desc': 'Powered by Vditor 3.x with three editing modes — WYSIWYG, Instant Rendering (IR), and Split View (SV). Built-in math formulas, code highlighting, typewriter mode, real-time outline & word count.',
      ...
      'f2.desc': 'A powerful command palette covering all operations: edit commands, format commands, view commands, file commands, plus menu commands, plugin commands and shortcut cheatsheet. Recent usage smart-sorted.',
      ...
      'f3.desc': 'Markmap-powered mindmap visualization of your document structure. Standalone window with free zoom levels. Combined with upcoming Wikilinks and backlinks features.',
      ...
      'f4.desc': '7 carefully crafted built-in themes, compatible with importing Typora theme CSS files. Editor theme can be set independently from app appearance.',
      ...
      'rm.r2Desc': 'Paste/drag upload, image host, multiple storage modes, compression, cropping, and batch management.',
      ...
      'dl.tagIntel': 'Intel + Apple Silicon',
      'dl.tagDeb': '.deb · AppImage',
      'dl.winFormat': 'NSIS Installer · 64-bit',
      'dl.ctaWin': 'Download .exe',
      'dl.winReq': 'Windows 10 or later (Windows 11 recommended)',
      'dl.macFormat': '.dmg universal installer',
      'dl.ctaMac': 'Download .dmg',
      'dl.macReq': 'macOS 11 Big Sur or later',
      'dl.linuxFormat': 'Debian-based distributions',
      'dl.ctaLinux': 'Download .deb',
      'dl.linuxReq': 'Ubuntu 20.04 LTS or later',
  ...
  function initMobileMenu() {
    const hb = $('#hamburger'), links = $('#navLinks'), overlay = $('#navOverlay');
    if (!hb || !links) return;
    ...
    $$('#navLinks a').forEach(link => link.addEventListener('click', closeMobileMenu));
  }
  ...
  function initScrollSpy() {
    ...
    const secs = $$('main section[id]'), navLks = $$('#navLinks a[href^="#"]');
